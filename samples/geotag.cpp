...
std::string Position::toExifString(double d,bool bRational,bool bLat)
{
    ...
    char result[200];
    if ( bRational )
        sprintf(result,"%d/1 %d/1 %d/1" ,deg,min,sec_int);
    else
        sprintf(result,"%03d%s%02d'%02d\"%s" ,deg,gDeg,min,sec_int,NSEW);

    std::cout << "Position::toExifString: " << deg << ", "<< min << ", "<< sec << std::endl;
    return std::string(result);
}
...
int main(int argc, char* const argv[])
{
    ...
    stringvec v;
    string rtDir = argv[1];
    string oxtsDataDir = rtDir + "/oxts/data";
    string imagesDir = rtDir + "/image_02/data";

    // read_directory(oxtsDataDir, v);
    std::ifstream fTimes;
    int n = 0;
    string strPathTimeFile = rtDir + "/oxts" + "/timestamps.txt";
    ...
    std::cout << n << " images" << std::endl;
    for(int i=0; i<n; i++)
    {
        std::stringstream ss;
        ss << oxtsDataDir << "/" << setfill('0') << setw(10) << i << ".txt";

        // Define variables.
        std::ifstream istrm(ss.str());
        double lat = 0.0f, lon = 0.0f;
        if(istrm.is_open())
        {
            istrm >> std::setprecision(std::numeric_limits<long double>::digits10 + 1) >> lat >> lon;
            std::cout << std::setprecision(std::numeric_limits<long double>::digits10 + 1) << lat << "," << lon << std::endl;

            istrm.close();            
        }

        std::stringstream imagess;
        imagess << imagesDir << "/" << setfill('0') << setw(10) << i << ".jpg";

        Exiv2::Image::AutoPtr image = Exiv2::ImageFactory::open(imagess.str());
        assert(image.get() != 0);
        image->readMetadata();
        ...
        exifData["Exif.GPSInfo.GPSLatitude"         ] = Position::toExifString(lat,true,true);
        exifData["Exif.GPSInfo.GPSLongitude"        ] = Position::toExifString(lon,true,false);
        exifData["Exif.GPSInfo.GPSLatitudeRef"      ] = lat>0?"N":"S";
        exifData["Exif.GPSInfo.GPSLongitudeRef"     ] = lon>0?"E":"W";
        exifData["Exif.GPSInfo.GPSAltitude"         ] = Position::toExifString(10.0);
